...
async function getPropertyData(slug) {
  try {
    const res = await fetch(`${API_URL}/api/propiedades?filters[slug][$eq]=${slug}&filters[Country]=US&populate[Gallery][fields][0]=url&populate[TitlePageSliderDesktop][fields][0]=url`, {
      cache: 'no-store',
    });
...
export default async function Page({ params }) {
  ...
  return (
    <Inside item={property} isMX={false} />
  );
}
